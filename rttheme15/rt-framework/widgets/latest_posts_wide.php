...
class Latest_Posts_wide extends WP_Widget {

	function Latest_Posts_wide() {
		$opts =array(
					'classname' 	=> 'widget_latest_posts_wide',
					'description' 	=> __( 'The most recent posts on your site with post thumbnails.', 'rt_theme_admin' )
				);

		$this-> WP_Widget('latest_posts_wide', '['. THEMENAME.']   '.__('Recent Posts Wide ', 'rt_theme_admin'), $opts);
	}
	...
	function widget( $args, $instance ) {
		...
		if ($post_query->have_posts()) : while ($post_query->have_posts()) : $post_query->the_post();
			...
			if ( has_post_thumbnail() && !$show_thumbnails) :
				...
				$f_image 		= 	@vt_resize( $f_thumb, '', 200, 65, true );			
